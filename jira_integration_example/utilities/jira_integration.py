...
This module defines a function to interact with Jira when a
...
def update_jira_based_on_monitoring_notification(jira_client, jira_project,
    ...
    Args:
        jira_client: A JIRA object that acts as a client which allows
            ...
            the new Jira issue.
        jira_project: A key or id of the Jira project under which to create
            the Jira issue.
    ...
    if incident_state == 'open':
        summary = '%s - %s' % (incident_condition_name,
                               incident_resource_name)
        description = '%s\nSee: %s' % (incident_summary,
                                       incident_url)
        ...
        raise UnknownConditionStateError(
            f'Condition state must be "open" or "closed"; actual: {condition_state}')